import { StatComparison, TeamStats } from "../api/client";
      ...
      {comparison.map((row) => {
        ...
        return (
          <div key={row.key} className="stat-row">
            <span className={`stat-val left ${isTeam1Better ? "winner" : ""}`}>{row.team1Value}</span>
            ...
            <span className={`stat-val right ${!isTeam1Better ? "winner" : ""}`}>{row.team2Value}</span>
          </div>
        );
      })}
    </div>
